...
- セッション起動直後は、上記参照順の正本ドキュメントを冒頭で一度すべて確認する。
...
- 各ターン冒頭の優先度提示（高/中/低）は、方針決定・調査開始・作業計画確定が必要な場面に限定して行う。
- すでに方針が確定している実行フェーズ、単純な質疑応答、進捗報告のみの応答では優先度提示を省略する。
...
- ブランチ切り替え前に、変更の「見た目」ではなく「目的（UI/E2E/認証/docs/infra）」で分類して作業先を決める。分類時は `git status --short` に加えて、サブモジュール差分がある場合は参照先コミットの件名（例: `git -C .cursor log -1 --oneline`）まで確認してから判断する。
- ブランチ切り替え直後は `git status -sb` を必ず確認し、意図しない持ち越し差分があれば作業を進める前に解消方針を明示する。
- 新規実装ブランチは、`main` 最新化後の `main` から作成する。未マージの作業ブランチから新規ブランチを派生させない（例外: stacked PR をユーザー合意済みの場合のみ）。
- 分岐元確認とPR順序の詳細手順は `.cursor/docs/branch-strategy.md` を正本として参照し、`AGENTS.md` には詳細を書かない。
...
- `.cursor/docs` に新規 `*.md` を追加する場合は、先頭に「概要（目的/対象/非対象）」を明記し、ファイル名と本文テーマが一致していることを確認してから保存する。
- `.cursor/docs/*.md` を編集したターンでは、空ファイル化やタイトル不一致がないかを最終確認する。
